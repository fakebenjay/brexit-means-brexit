...
function horizBar(d, propsObj) {
  Promise.all([d3.csv('pandas/output/spending.csv'), d, propsObj]).then(
    function (data) {
      ...
      for (let i = 0; i < councilsAll.length; i++) {
        ...
        if (
          councilsAll[i].pct_remain - decade >= 10 ||
          i === councilsAll.length - 1
        ) {
...
function drawPie(d, i) {
  ...
  Promise.all([d]).then(function (data) {
    var properties = data[0].properties;

    d3.selectAll('#map-div input').on('click', update);
    ...
    function update(val = this.value) {
      ...
      legendSVG
        ...
        .attr(
          'x',
          d => `${legendXScale(d.pctRemain) + legendMargin - legendOffset}px`
        )
        .attr(
          'y',
      ...
      var piePath = pieSVG.selectAll('path').data(pie(results));

      var labelArcs = pieSVG.selectAll('text').data(pie(results));
...
var barElectorateSVG = d3
  .select('#pct-bar-electorate')
  .append('svg')
  .attr('class', 'bar-svg')
  .attr('width', `100%`)
  .attr('height', '100%')
...
var barWidthScalePC = d3
  .scaleLinear()
  .domain([0, 1013925.169])
  .range([0, wBar * 0.9]);

var barWidthScaleElectorate = d3
  .scaleLinear()
  .domain([0, 46475882])
  .range([0, wBar * 0.9]);

var barWidthScaleTotal = d3
  .scaleLinear()
...
var pieSVG = d3
  ...
  .attr(
    'transform',
    `translate(${Math.min(wPie, hPie) / 2},${Math.min(wPie, hPie) / 2})`
  );

var w = document.querySelector('#map-div').offsetWidth;
var h = document.querySelector('#map-div').offsetHeight * 0.99;

var select = d3.select('div#select-div').append('select').attr('id', 'select');
...
var legendXScale = d3.scaleLinear().domain([20, 80]).range([0, wLegend]);

var legendYScale = d3.scaleLinear().domain([100, 0]).range([0, hLegend]);
...
var mapLegend = d3
  ...
  .style(
    'margin',
    `${parseFloat(legendMargin)}px ${parseFloat(legendMargin)}px ${parseFloat(
      legendMargin
    )}px ${parseFloat(legendMargin)}px`
  );
...
var legendBars = d3
  ...
  .style(
    'background-image',
    d => `linear-gradient(${d.maxColor}, ${d.minColor})`
  );
...
var ptileOpacity = d3.scaleLinear().domain([0, 100]).range([0.1, 1.0]);

var spendingOpacity = d3
  .scaleLinear()
...
function regionName(name) {
  if (name === 'East') {
    ...
    (name.includes('North') || name.includes('South')) &&
    !name.includes('Ireland')
  ) {
    return `${name} England`;
  } else {
    return name;
...
function init(data) {
  ...
  var mapJSON = topojson.feature(json, json.objects['local-areas']).features;
  var regionMesh = topojson.mesh(json, json.objects['local-regions']);
  ...
  for (let i = 0; i < stats.length; i++) {
    ...
    let projEUContribution = parseFloat(
      stats[i].project_eu_contribution_gbp
    ).toFixed(2);
  ...
  legendSVG
    ...
    .attr(
      'class',
      d => `${d.properties.region.toLowerCase().replaceAll(' ', '-')} scatter`
    )
    ...
    .on('mouseover', function (d) {
      ...
      d3.selectAll('.scatter').style('display', 'none');
      d3.selectAll(`.scatter.${regionClass}`).style('display', 'block');
      d3.selectAll('.region-path').style('opacity', 0);
      d3.selectAll(`.region-path.${regionClass}`).style('opacity', 1);

      var coordinates = [
        parseFloat(this.attributes.cx.value.replaceAll('px', '')) +
          parseFloat(this.attributes.r.value.replaceAll('px', '')) -
          legendMargin,
        parseFloat(this.attributes.cy.value.replaceAll('px', '')) +
          parseFloat(this.attributes.r.value.replaceAll('px', '')) -
          legendMargin
      ];
      ...
      var vertOffset =
        document.querySelector('#legend-tooltip').offsetHeight / 2;
      // var horizOffset = document.querySelector('#legend-tooltip').offsetWidth;

      d3.select('#legend-tooltip').select('.region').text(d.properties.region);

      d3.select('#legend-tooltip')
        .style('top', `${coordinates[1] - vertOffset}px`)
        .style('left', `${coordinates[0] + 10}px`);

      d3.select('#legend-tooltip').classed('hidden', false);
    ...
    .on('mouseout', function () {
      d3.selectAll('.scatter').style('display', 'block');
      d3.selectAll('.region-path').style('opacity', 1);
      d3.select('#legend-tooltip').classed('hidden', true);

      var that = d3.select(this);
      if (that.attr('class').includes('scatter-clicked')) {
        that.transition(200).style('fill', '#F7C603');
      } else {
        that
          .transition(200)
          .attr('r', `1px`)
  ...
  mapSVG
    ...
    .attr(
      ...
      d =>
        `${d.properties.region
          .toLowerCase()
          .replaceAll(' ', '-')} region-path unclicked`
    )
    .style('fill', d => {
    ...
    .on('click', function (d, i) {
      ...
      document.querySelector('#remain-voters > span').innerText = numeral(
        d.properties.remain
      ).format('0,0');
      document.querySelector('#leave-voters > span').innerText = numeral(
        d.properties.leave
      ).format('0,0');
      document.querySelector('#turnout > span').innerText = numeral(
        d.properties.pctTurnout / 100
      ).format('0.[00]%');
      document.querySelector('#valid-votes > span').innerText = numeral(
        d.properties.validVotes
      ).format('0,0');
      document.querySelector('#votes-cast > span').innerText = numeral(
        d.properties.votesCast
      ).format('0,0');
      document.querySelector('#rejected-ballots > span').innerText = numeral(
        d.properties.rejectedBallots
      ).format('0,0');

      if (d.properties.pctLeave > d.properties.pctRemain) {
        document.querySelector(
          '#vote-rank span'
        ).innerHTML = `<strong>${numeral(d.properties.leaveRank).format(
          '0o'
        )} most Leave</strong><br>of 270 Leave districts<br>(398 total)`;
      } else {
        document.querySelector(
          '#vote-rank span'
        ).innerHTML = `<strong>${numeral(d.properties.remainRank).format(
          '0o'
        )} most Remain</strong><br>of 128 Remain districts<br>(398 total)`;
      }

      document.querySelector('span#percap-spend').innerText = `£${numeral(
        d.properties.spendingPerVoter
      ).format('£0,0.00')} (Ranked ${numeral(
        parseInt(d.properties.percapRank)
      ).format('0o')}, ${numeral(parseInt(d.properties.percentile)).format(
        '0o'
      )} percentile, ${numeral(
      ...
      //of £1,013,925.17
      //median £443.0366
      document.querySelector('span#total-electorate').innerText = `${numeral(
        d.properties.electorate
      ).format('0,0')} (Ranked ${numeral(
        parseInt(d.properties.electorateRank)
      ).format('0o')}, ${numeral(
        parseInt(d.properties.electorate) / 46475882
      ).format('0.[00]%')} of 46,475,882)`;
      document.querySelector('span#total-spend').innerText = `£${numeral(
        d.properties.projEUContribution
      ).format('£0,0.00')} (Ranked ${numeral(
        parseInt(d.properties.projEURank)
      ).format('0o')}, ${numeral(
        parseFloat(d.properties.projEUContribution) / 100321726921.54
      ).format('0.[000]%')} of £100,321,726,921.54)`;
      d.properties.electorate;
    })
    .on('mouseover', function (d) {
      ...
      d3.select('#map-tooltip').select('.council').text(d.properties.LAD13NM);

      d3.select('#map-tooltip').select('.region').text(d.properties.region);
    ...
    .on('mouseout', () => {
      d3.selectAll('circle:not(.scatter-clicked)')
        .attr('r', `1px`)
        .style('fill', 'black')
        .style('stroke-width', '0px');
